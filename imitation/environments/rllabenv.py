...
import lab
from lab import spaces, envs

lab.undo_logger_setup()
import logging; logging.getLogger('lab.core').addHandler(logging.NullHandler())


class RLLabSim(policyopt.Simulation):
    ...
    def step(self, action):
        if isinstance(self.action_space, spaces.Discrete):
            ...
            # but Lab wants the integer itself
...
def _convert_space(space):
    '''Converts a rl-lab space to our own space representation'''
...
class RLLabMDP(policyopt.MDP):
    def __init__(self, env_name):
        print 'Lab version:', lab.version.VERSION
