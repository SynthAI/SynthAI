import lab
from lab import Wrapper
from lab import error, version, logger
import os, json, numpy as np, six
from lab.wrappers.monitoring import stats_recorder, video_recorder
from lab.utils import atomic_write, closer
from lab.utils.json_utils import json_encode_np

FILE_PREFIX = 'synthailab'
...
class Monitor(Wrapper):
    ...
    def _start(self, directory, video_callable=None, force=False, resume=False,
        ...
        Args:
            ...
            force (bool): Clear out existing training data from this directory (by deleting every file prefixed with "synthailab.").
        ...
        if self.env.spec is None:
            logger.warn("Trying to monitor an environment which has no 'spec' set. This usually means you did not create it via 'lab.make', and is recommended only for advanced users.")
            env_id = '(unknown)'
        else:
 ...
 You should use a unique directory for each training run, or use 'force=True' to automatically clear previous monitor files.'''.format(directory, ', '.join(training_manifests[:5])))
        ...
        # We use the 'synthai-lab' prefix to determine if a file is
        # ours
    ...
    def close(self):
        ...
        logger.info('''Finished writing results. You can upload them to the scoreboard via lab.upload(%r)''', self.directory)
...
# not intended for use outside of the lab codebase.
def _open_monitors():
...
def collapse_env_infos(env_infos, training_dir):
    ...
    for key in ['env_id', 'lab_version']:
        if key not in first:
            raise error.Error("env_info {} from training directory {} is missing expected key {}. This is unexpected and likely indicates a bug in lab.".format(first, training_dir, key))
    return first