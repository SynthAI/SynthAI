...
**SynthAI Lab is a toolkit for developing and comparing reinforcement learning algorithms.** This is the ``lab`` open-source library, which gives you access to an ever-growing variety of environments.

.. image:: https://travis-ci.org/synthai/lab.svg?branch=master
    :target: https://travis-ci.org/synthai/lab

`See What's New section below <#what-s-new>`_

``lab`` makes no assumptions about the structure of your agent, and is compatible with any numerical computation library, such as TensorFlow or Theano. You can use it from Python code, and soon from other languages.

If you're not sure where to start, we recommend beginning with the
`docs <https://lab.synthai.com/docs>`_ on our site. See also the `FAQ <https://github.com/synthai/lab/wiki/FAQ>`_.

A whitepaper for SynthAI Lab is available at http://arxiv.org/abs/1606.01540, and here's a BibTeX entry that you can use to cite it in a publication::
...
The core `lab` interface is `Env <https://github.com/synthai/lab/blob/master/lab/core.py>`_, which is
...
You can perform a minimal install of ``lab`` with:

.. code:: shell

	  git clone https://github.com/synthai/lab.git
	  cd lab
	  pip install -e .
...
- `algorithmic <https://lab.synthai.com/envs#algorithmic>`_
- `toy_text <https://lab.synthai.com/envs#toy_text>`_
- `classic_control <https://lab.synthai.com/envs#classic_control>`_ (you'll need ``pyglet`` to render though)
...
the
`instructions <https://github.com/synthai/mujoco-py#obtaining-the-binaries-and-license-key>`_
in the ``mujoco-py`` package for help.

Once you're ready to install everything, run ``pip install -e '.[all]'`` (or ``pip install 'lab[all]'``).
...
If you want to access Lab from languages other than python, we have limited support for non-python
frameworks, such as lua/Torch, using the SynthAI Lab `HTTP API <https://github.com/synthai/lab-http-api>`_.
...
<https://github.com/synthai/lab/blob/master/setup.py>`_ and
...
<https://github.com/synthai/lab/blob/master/setup.py>`_. We
...
`lab/envs
<https://github.com/synthai/lab/blob/master/lab/envs>`_. The
specification of each task is in `lab/envs/__init__.py
<https://github.com/synthai/lab/blob/master/lab/envs/__init__.py>`_. It's
...
<https://github.com/synthai/mujoco-py#obtaining-the-binaries-and-license-key>`_
to set it up. You'll have to also run ``pip install -e '.[mujoco]'`` if you didn't do the full install.

.. code:: python

	  import lab
	  env = lab.make('Humanoid-v1')
	  env.reset()
	  env.render()
...
- Run `examples/agents/random_agent.py <https://github.com/synthai/lab/blob/master/examples/agents/random_agent.py>`_ to run an simple random agent and upload the results to the scoreboard.
- Run `examples/agents/cem.py <https://github.com/synthai/lab/blob/master/examples/agents/cem.py>`_ to run an actual learning agent (using the cross-entropy method) and upload the results to the scoreboard.
- Run `examples/scripts/list_envs <https://github.com/synthai/lab/blob/master/examples/scripts/list_envs>`_ to generate a list of all environments. (You see also just `browse <https://lab.synthai.com/docs>`_ the list on our site.
- Run `examples/scripts/upload <https://github.com/synthai/lab/blob/master/examples/scripts/upload>`_ to upload the recorded output from ``random_agent.py`` or ``cem.py``. Make sure to obtain an `API key <https://lab.synthai.com/settings/profile>`_.
...
- 2017-05-13: BACKWARDS INCOMPATIBILITY: The Atari environments are now at
  *v4*. To keep using the old v3 environments, keep lab <= 0.8.2 and atari-py
...
- 2017-03-05: BACKWARDS INCOMPATIBILITY: The `configure` method has been removed
  from `Env`. `configure` was not used by `lab`, but was used by some dependent
  libraries including `institute`. These libraries will migrate away from the
  configure method by using wrappers instead. This change is on master and will be released with 0.8.0.
- 2016-12-27: BACKWARDS INCOMPATIBILITY: The lab monitor is now a
...
- 2016-09-21: Switch the Lab automated logger setup to configure the
  root logger rather than just the 'lab' logger.
