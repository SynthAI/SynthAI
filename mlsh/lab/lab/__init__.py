...
from lab.configuration import logger_setup, undo_logger_setup
from lab.utils import reraise
from lab.version import VERSION as __version__

logger = logging.getLogger(__name__)

# Do this before importing any other lab modules, as most of them import some
...
def sanity_check_dependencies():
    ...
    if distutils.version.LooseVersion(numpy.__version__) < distutils.version.LooseVersion('1.10.4'):
        logger.warn("You have 'numpy' version %s installed, but 'lab' requires at least 1.10.4. HINT: upgrade via 'pip install -U numpy'.", numpy.__version__)

    if distutils.version.LooseVersion(requests.__version__) < distutils.version.LooseVersion('2.0'):
        logger.warn("You have 'requests' version %s installed, but 'lab' requires at least 2.0. HINT: upgrade via 'pip install -U requests'.", requests.__version__)
...
#
# (Note: this code runs before importing the rest of lab, since we may
...
# (generally by caling `lab.undo_logger_setup()`), but in exchange,
# the library becomes much more usable for the uninitiated.
#
# Lab's design goal generally is to be simple and intuitive, and while
# the tradeoff is definitely not obvious in this case, we've come down
# on the side of auto-configuring the logger.

if not os.environ.get('LAB_NO_LOGGER_SETUP'):
    logger_setup()
del logger_setup

sanity_check_dependencies()

from lab.core import Env, Space, Wrapper, ObservationWrapper, ActionWrapper, RewardWrapper
from lab.benchmarks import benchmark_spec
from lab.envs import make, spec
from lab.scoreboard.api import upload
