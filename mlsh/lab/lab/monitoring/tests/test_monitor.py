...
import lab
from lab import error, spaces
from lab import monitoring
from lab.monitoring.tests import helpers
from lab.wrappers import Monitor
...
def test_semisuper_succeeds():
    ...
    with helpers.tempdir() as temp:
        env = lab.make('SemisuperPendulumDecay-v0')
        env = Monitor(env, temp)
        env.reset()
        env.step(env.action_space.sample())
        env.close()

class AutoresetEnv(lab.Env):
...
lab.envs.register(
    id='Autoreset-v0',
    entry_point='lab.monitoring.tests.test_monitor:AutoresetEnv',
    max_episode_steps=2,
)
def test_env_reuse():
    with helpers.tempdir() as temp:
        env = lab.make('Autoreset-v0')
        env = Monitor(env, temp)
...
def test_steps_limit_restart():
    with helpers.tempdir() as temp:
        env = lab.make('test.StepsLimitCartpole-v0')
        env = Monitor(env, temp, video_callable=False)
        env.reset()
