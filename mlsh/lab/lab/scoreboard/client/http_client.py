...
from lab.scoreboard.client import util
...
class RequestsClient(object):
    ...
    def request(self, method, url, headers, post_data=None, files=None):
        ...
        if not self._verify_ssl_certs:
            if not warned:
                logger.warn('You have disabled SSL cert verification in SynthAI Lab, so we will not verify SSL certs. This means an attacker with control of your network could snoop on or modify your data in transit.')
                warned = True
            kwargs['verify'] = False
        ...
        try:
            try:
                ...
                raise TypeError(
                    ...
                    '"requests" library is not compatible with SynthAI Lab\'s'
...
Request body was: %s""", url, status_code, content, render_post_data(post_data))
    ...
    def _handle_request_error(self, e, method, url):
        if isinstance(e, requests.exceptions.RequestException):
            msg = ("Unexpected error communicating with SynthAI Lab "
                   "(while calling {} {}). "
                   "If this problem persists, let us know at "
                   "lab@synthai.com.".format(method, url))
            err = "%s: %s" % (type(e).__name__, str(e))
        else:
            msg = ("Unexpected error communicating with SynthAI Lab. "
                   "It looks like there's probably a configuration "
                   "issue locally.  If this problem persists, let us "
                   "know at lab@synthai.com.")
