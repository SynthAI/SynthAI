...
from lab.scoreboard.client import api_requestor, util

def convert_to_lab_object(resp, api_key):
    types = {
    ...
    if isinstance(resp, list):
        return [convert_to_lab_object(i, api_key) for i in resp]
    elif isinstance(resp, dict) and not isinstance(resp, LabObject):
        resp = resp.copy()
        klass_name = resp.get('object')
        if isinstance(klass_name, string_types):
            klass = types.get(klass_name, LabObject)
        else:
            klass = LabObject
...
class LabObject(dict):
    ...
    def update(self, update_dict):
        ...
        return super(LabObject, self).update(update_dict)

    def __setattr__(self, k, v):
        if k[0] == '_' or k in self.__dict__:
            return super(LabObject, self).__setattr__(k, v)
        else:
            self[k] = v
    ...
    def __delattr__(self, k):
        if k[0] == '_' or k in self.__dict__:
            return super(LabObject, self).__delattr__(k)
        else:
            del self[k]
    ...
    def __setitem__(self, k, v):
        ...
        super(LabObject, self).__setitem__(k, v)
    ...
    def __getitem__(self, k):
        try:
            return super(LabObject, self).__getitem__(k)
        except KeyError as err:
            if k in self._transient_values:
                raise KeyError(
                    "%r.  HINT: The %r attribute was set in the past."
                    "It was then wiped when refreshing the object with "
                    "the result returned by Rl_Lab's API, probably as a "
    ...
    def __delitem__(self, k):
        super(LabObject, self).__delitem__(k)
    ...
    def refresh_from(self, values, api_key=None, partial=False):
        ...
        for k, v in iteritems(values):
            super(LabObject, self).__setitem__(
                k, convert_to_lab_object(v, api_key))
    ...
    def request(self, method, url, params=None, headers=None):
        ...
        return convert_to_lab_object(response, api_key)
    ...
    def to_dict(self):
        warnings.warn(
            ...
            'version 2.0 of the Rl_Lab bindings. The LabObject is '
...
class FileUpload(ListableAPIResource):
    ...
    def put(self, contents, encode='json'):
        ...
        if code != 204:
            raise error.Error("Upload to S3 failed. If error persists, please contact us at lab@synthai.com this message. S3 returned '{} -- {}'. Tried 'POST {}' with fields {}.".format(code, body, self.post_url, self.post_fields))

class Evaluation(CreateableAPIResource):
    def web_url(self):
        return "%s/evaluations/%s" % (lab.scoreboard.web_base, self.get('id'))
...
class BenchmarkRun(CreateableAPIResource, UpdateableAPIResource):
    ...
    def web_url(self):
        return "%s/benchmark_runs/%s" % (lab.scoreboard.web_base, self.get('id'))
