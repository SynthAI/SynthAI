...
import tarfile
import tempfile
from lab import benchmark_spec, error, monitoring
from lab.scoreboard.client import resource, util
import numpy as np
...
def upload(training_dir, algorithm_id=None, writeup=None, tags=None, benchmark_id=None, api_key=None, ignore_open_monitors=False, skip_videos=False):
    ...
    env's monitor) to SynthAI Lab.

    Args:
        training_dir (str): A directory containing the results of a training run.
        algorithm_id (Optional[str]): An algorithm id indicating the particular version of the algorithm (including choices of parameters) you are running (visit https://lab.synthai.com/algorithms to create an id). If the id doesn't match an existing server id it will create a new algorithm using algorithm_id as the name
        benchmark_id (Optional[str]): The benchmark that these evaluations belong to. Will recursively search through training_dir for any Lab manifests. This feature is currently pre-release.
        ...
        api_key (Optional[str]): Your SynthAI API key. Can also be provided as an environment variable (SYNTHAI_LAB_API_KEY).
...
SynthAI Lab! You can find it at:
...
def _upload_benchmark(training_dir, algorithm_id, benchmark_id, benchmark_run_tags, api_key, ignore_open_monitors, skip_videos):
    ...
    try:
        ...
        raise error.Error("Invalid benchmark id: {}. Are you using a benchmark registered in lab/benchmarks/__init__.py?".format(benchmark_id))
...
def _upload(training_dir, algorithm_id=None, writeup=None, benchmark_run_id=None, api_key=None, ignore_open_monitors=False, skip_videos=False):
    ...
    if logger.level <= logging.INFO:
        if training_episode_batch_id is not None and training_video_id is not None:
            ...
            raise error.Error("[%s] You didn't have any recorded training data in %s. Once you've used 'env = lab.wrappers.Monitor(env, directory)' to start recording, you need to actually run some rollouts. Please join the community chat on https://lab.synthai.com if you have any issues."%(env_id, training_dir))
    ...
    evaluation = resource.Evaluation.create(
        ...
        writeup=writeup,
        lab_version=env_info['lab_version'],
        api_key=api_key,
    )
...
def upload_training_video(videos, api_key=None, env_id=None):
    ...
    with tempfile.TemporaryFile() as archive_file:
        ...
        file_upload = resource.FileUpload.create(purpose='video', content_type='application/vnd.synthai.video+x-compressed', api_key=api_key)
...
def write_archive(videos, archive_file, env_id=None):
    if len(videos) > MAX_VIDEOS:
        raise error.Error('[{}] Trying to upload {} videos, but there is a limit of {} currently. If you actually want to upload this many videos, please email lab@synthai.com with your use-case.'.format(env_id, MAX_VIDEOS, len(videos)))
