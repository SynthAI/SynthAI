...
def score_from_local(directory):
    ...
    results = lab.monitoring.load_results(directory)
    # No scores yet saved
    if results is None:
        return None
    ...
    spec = lab.spec(results['env_info']['env_id'])
...
def benchmark_score_from_local(benchmark_id, training_dir):
    spec = lab.benchmark_spec(benchmark_id)

    directories = []
    for name, _, files in os.walk(training_dir):
        manifests = lab.monitoring.detect_training_manifests(name, files=files)
        if manifests:
            directories.append(name)

    benchmark_results = defaultdict(list)
    for training_dir in directories:
        results = lab.monitoring.load_results(training_dir)
    ...
    return lab.benchmarks.scoring.benchmark_aggregate_score(spec, benchmark_results)
...
def compute_graph_stats(episode_lengths, episode_rewards, timestamps, initial_reset_timestamp, buckets):
    ...
    # Not a dependency of SynthAI Lab generally.
    import scipy.stats
