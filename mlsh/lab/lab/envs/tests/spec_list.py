...
def should_skip_env_spec_for_tests(spec):
    ...
    if skip_mujoco and ep.startswith('lab.envs.mujoco:'):
        return True
    if (    'GoEnv' in ep or
            'HexEnv' in ep or
            ep.startswith('lab.envs.box2d:') or
            ep.startswith('lab.envs.box2d:') or
            ep.startswith('lab.envs.parameter_tuning:') or
            ep.startswith('lab.envs.safety:Semisuper') or
            (ep.startswith("lab.envs.atari") and not spec.id.startswith("Pong") and not spec.id.startswith("Seaquest"))
    ):
