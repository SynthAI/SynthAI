...
std::string glsl_path = "synthtraining /cpp-household/glsl"; // outside namespace
...
//		//_texture_paint( cx->cached_bind_texture("synthtraining /models_furniture/IKEA_denimcloth.jpg") );
...
void opengl_init_before_app(const boost::shared_ptr<Household::World>& wref)
{
	...
	QApplication::setApplicationDisplayName("Synthtraining ");
}
...
void opengl_init(const boost::shared_ptr<Household::World>& wref)
{
	...
	if (1) {
		...
		if (!ok_without_shadows) {
			...
			fprintf(stderr, "For possible fixes, see:\n\nhttps://github.com/synthai/synthtraining /issues/2\n\n");
			assert(0);
		}
		cx->glcx = glcx;
