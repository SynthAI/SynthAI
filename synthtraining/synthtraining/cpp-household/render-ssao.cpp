...
bool Context::_hbao_init()
{
	...
	if (!program_depth_linearize->log().isEmpty()) {
		fprintf(stderr, "Synthtraining  built-in render compiled with shadows, but SSAO shaders didn't load (1)\n");
		return false;
	}
	...
	if (!program_hbao_calc->log().isEmpty()) {
		fprintf(stderr, "Synthtraining  built-in render compiled with shadows, but SSAO shaders didn't load (2)\n");
		return false;
	}
