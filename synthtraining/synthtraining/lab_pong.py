from synthtraining .scene_abstract import Scene, cpp_household
from synthtraining .multiplayer import SharedMemoryClientEnv
...
class PongScene(Scene):
    ...
    def episode_restart(self):
        ...
        if self.score_right + self.score_left > 0:
            ...
            sys.stdout.flush()
        self.mjcf = self.cpp_world.load_mjcf(os.path.join(os.path.dirname(__file__), "models_robot/synthtraining _pong.xml"))
        dump = 0
        for r in self.mjcf:
...
class Synthtraining Pong(lab.Env, SharedMemoryClientEnv):
    metadata = {
    ...
    def __init__(self):
        ...
        self.action_space = lab.spaces.Box(-high, high)
        high = np.inf*np.ones([obs_dim])
        self.observation_space = lab.spaces.Box(-high, high)
        self._seed()
    ...
    def _seed(self, seed=None):
        self.np_random, seed = lab.utils.seeding.np_random(seed)
        return [seed]
