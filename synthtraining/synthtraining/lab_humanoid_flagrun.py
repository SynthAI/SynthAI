...
class Synthtraining HumanoidFlagrun(Synthtraining Humanoid):
    def create_single_player_scene(self):
        s = Synthtraining Humanoid.create_single_player_scene(self)
    ...
    def calc_state(self):
        self.flag_timeout -= 1
        state = Synthtraining Humanoid.calc_state(self)
        if self.walk_target_dist < 1 or self.flag_timeout <= 0:
            self.flag_reposition(first=False)
            state = Synthtraining Humanoid.calc_state(self)  # caclulate state again, against new flag pos
...
class Synthtraining HumanoidFlagrunHarder(Synthtraining HumanoidFlagrun):
    def __init__(self):
        Synthtraining HumanoidFlagrun.__init__(self)
        self.underlearned = RepeatUnderlearnedTasks(self.TASKS)

    def robot_specific_reset(self):
        Synthtraining HumanoidFlagrun.robot_specific_reset(self)
    ...
    def alive_bonus(self, z, pitch):
        ...
        if z < 0.8:
            ...
            self.electricity_cost = Synthtraining Humanoid.electricity_cost / 5.0   # Don't care about electricity, just stand up!
        elif self.on_ground_frame_counter > 0:
            self.on_ground_frame_counter -= 1
            self.electricity_cost = Synthtraining Humanoid.electricity_cost / 2.0    # Same as in Flagrun
        else:
            self.walking_normally += 1
            self.electricity_cost = Synthtraining Humanoid.electricity_cost / 2.0
    ...
    def calc_potential(self):
        ...
        flag_running_progress = Synthtraining Humanoid.calc_potential(self)
