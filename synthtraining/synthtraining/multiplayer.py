...
class SharedMemoryClientEnv:
    ...
    Environment that support multiplayer inherite from this class. If you call lab.make("Synthtraining Hopper-v0")
    ...
    def shmem_client_init(self, game_server_guid, player_n):
        ...
        """
        assert isinstance(self.observation_space, lab.spaces.Box)
        assert isinstance(self.action_space, lab.spaces.Box)
    ...
    def multiplayer(self, env, game_server_guid, player_n):
        """
        That's the function you call between lab.make() and first env.reset(), to connect to multiplayer server.
...
class SharedMemoryPlayerAgent:
    ...
    def read_env_id_and_create_env(self):
        ...
        self.env = lab.make(env_id)  # lab.make() creates at least timeout wrapper, we need it.

        self.env.unwrapped.scene = self.scene
        self.env.unwrapped.player_n = self.player_n
        assert isinstance(self.env.observation_space, lab.spaces.Box)
        assert isinstance(self.env.action_space, lab.spaces.Box)
