...
blib = setup_py_dir + "/synthtraining /cpp-household/bullet_local_install/lib"
...
def recompile():
    ...
    cmd = "cd %s/synthtraining /cpp-household && make clean && make -j4 dirs %s ../cpp_household.so" % (setup_py_dir, USE_PYTHON3)
    print(cmd)
    res = os.system(cmd)
    if res:
        print(dep)
...
setup(
    name = 'synthtraining ',
    version = '1.0',
    description = 'SynthAI Household Simulator: mobile manipulation using Bullet',
    maintainer = 'Oleg Klimov',
    maintainer_email = 'omgtech@gmail.com',
    url = 'https://github.com/synthai/synthtraining ',
