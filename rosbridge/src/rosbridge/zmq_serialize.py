"""
Helpers to serialize Lab data types over zmq.
"""
import math, logging, random
import numpy as np
import ujson
import lab, lab.spaces
...
def dump_msg(msg):
    ...
    This is less general than Pickle, but it's:
        ...
        - Supports Lab objects like subtypes of lab.Space
    ...
    if isinstance(o, dict):
        ...
        if t is not None:
            if t == 'ndarray':
                ...
                return lab.spaces.Box(low=_load_msg1(o['low'], parts), high=_load_msg1(o['high'], parts))
            elif t == 'Tuple':
                return lab.spaces.Tuple(spaces=_load_msg1(o['spaces'], parts))
            elif t == 'Discrete':
                return lab.spaces.Discrete(n=_load_msg1(o['n'], parts))
            elif t == 'tuple':
