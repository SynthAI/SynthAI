...
import (
	...
	"github.com/synthai/go-vncdriver/vncclient"
)
...
func GoVNCDriver_VNCSession_c_init(self *C.go_vncdriver_VNCSession_object) C.int {
	...
	info := sessionInfo{
		...
		screenNumpy: map[string]map[*labvnc.Screen]*C.PyObject{},
	}
...
func GoVNCDriver_VNCSession_connect(self, args, kwds *C.PyObject) *C.PyObject {
	...
	*subsampleLevelC = C.int(-1)
	...
	err := info.batch.Open(name, labvnc.VNCSessionConfig{
		Address:  address,
		Password: password,
		Encoding: encoding,
	...
	// done := make(chan bool)
	// batch, err := labvnc.NewVNCSession(, )
	// if err != nil {
	// 	close(done)
	// 	setError(err)
	// 	return C.int(-1)
...
func GoVNCDriver_VNCSession_render(self, args, kwds *C.PyObject) *C.PyObject {
	...
	if !compiledWithGL {
		setError(errors.New("go_vncdriver was installed without OpenGL support. See https://github.com/synthai/go-vncdriver for details on how debug."))
		return nil
	}
...
// func reportBestError(batch *labvnc.VNCBatch, err error) {
// 	var report string
...
func convertSubscriptionPy(subscriptionPy *C.PyObject) (regions []labvnc.Region, ok bool) {
	ok = true
	...
	for itemPy := C.PyIter_Next(subscriptionIter); ok && itemPy != nil; itemPy = C.PyIter_Next(subscriptionIter) {
		...
		regions = append(regions, labvnc.Region{
			X:      uint16(x),
			Y:      uint16(y),
			Width:  uint16(width),
...
// Sets python error
func convertEventPy(eventPy *C.PyObject) (event labvnc.VNCEvent, ok bool) {
...
func (b *sessionInfo) open(name string) {
	...
	b.screenNumpy[name] = map[*labvnc.Screen]*C.PyObject{}
}
...
func (b *sessionInfo) populateScreenPyDict(screens map[string]*labvnc.Screen) bool {
