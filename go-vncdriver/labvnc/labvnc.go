...
var (
	log = logging.MustGetLogger("labvnc")
	id  = 0
)
...
func (c *VNCSession) Render(close bool) error {
	if c.renderer == nil {
		return errors.Errorf("[%s] VNCSession has no renderer. This likely means your go_vncdriver was installed without the OpenGL viewer. See https://github.com/synthai/go-vncdriver for details on how debug.", c.label)
	}
