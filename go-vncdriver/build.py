...
def build():
    ...
    os.makedirs(os.path.normpath('.build/src/github.com/synthai'))
    os.symlink('../../../..', '.build/src/github.com/synthai/go-vncdriver')
    ...
    def build_no_gl():
        cmd = 'go build -tags no_gl -buildmode=c-shared -o go_vncdriver.so github.com/synthai/go-vncdriver'
...
''')
    ...
    def build_gl():
        cmd = 'go build -buildmode=c-shared -o go_vncdriver.so github.com/synthai/go-vncdriver'
    ...
    if os.getenv('GO_VNCDRIVER_NOGL'):
        ...
        if not build_gl():
            eprint('\nNote: could not build with OpenGL rendering (cf https://github.com/synthai/go-vncdriver). This is expected on most servers. Going to try building without OpenGL.')
            build_no_gl()
