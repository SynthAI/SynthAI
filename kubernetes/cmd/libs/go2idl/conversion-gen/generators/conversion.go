...
func getPeerTypeFor(context *generator.Context, t *types.Type, potenialPeerPkgs []string) *types.Type {
	for _, ppp := range potenialPeerPkgs {
		p := context.Institute.Package(ppp)
		if p == nil {
			continue
		}
...
func Packages(context *generator.Context, arguments *args.GeneratorArgs) generator.Packages {
	...
	for i := range inputs {
		...
		if externalTypesValues != nil {
			...
			// update context.Order to the latest context.Institute
			orderer := namer.Orderer{Namer: namer.NewPublicNamer(1)}
			context.Order = orderer.OrderInstitute(context.Institute)
		}
		...
		for _, pp := range peerPkgs {
			context.AddDir(pp)
			getManualConversionFunctions(context, context.Institute[pp], manualConversions)
		}
...
func (g *genConversion) Init(c *generator.Context, w io.Writer) error {
	...
	scheme := c.Institute.Type(types.Name{Package: runtimePackagePath, Name: "Scheme"})
