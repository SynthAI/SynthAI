...
func construct(t *testing.T, files map[string]string, testNamer namer.Namer) (*parser.Builder, types.Institute, []*types.Type) {
	b := parser.New()
...
func testOpenAPITypeWritter(t *testing.T, code string) (error, *assert.Assertions, *bytes.Buffer) {
	...
	builder, institute, _ := construct(t, testFiles, rawNamer)
	...
	blahT := institute.Type(types.Name{Package: "base/foo", Name: "Blah"})
