...
func (g *factoryGenerator) GenerateType(c *generator.Context, t *types.Type, w io.Writer) error {
	...
	for groupName := range g.groupVersions {
		gvInterfaces[groupName] = c.Institute.Type(types.Name{Package: packageForGroup(vendorless(g.outputPackage), g.groupVersions[groupName].Group), Name: "Interface"})
		gvNewFuncs[groupName] = c.Institute.Function(types.Name{Package: packageForGroup(vendorless(g.outputPackage), g.groupVersions[groupName].Group), Name: "New"})
	}
	m := map[string]interface{}{
		...
		"interfacesNewInformerFunc":  c.Institute.Type(types.Name{Package: g.internalInterfacesPackage, Name: "NewInformerFunc"}),
		"informerFactoryInterface":   c.Institute.Type(types.Name{Package: g.internalInterfacesPackage, Name: "SharedInformerFactory"}),
		"clientSetInterface":         c.Institute.Type(types.Name{Package: g.clientSetPackage, Name: "Interface"}),
		"reflectType":                c.Institute.Type(reflectType),
		"runtimeObject":              c.Institute.Type(runtimeObject),
		"schemaGroupVersionResource": c.Institute.Type(schemaGroupVersionResource),
		"syncMutex":                  c.Institute.Type(syncMutex),
