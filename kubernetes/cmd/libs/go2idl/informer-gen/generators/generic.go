...
func (g *genericGenerator) GenerateType(c *generator.Context, t *types.Type, w io.Writer) error {
	...
	for _, groupVersions := range g.groupVersions {
		...
		for _, v := range groupVersions.Versions {
			...
			schemeGVs[version] = c.Institute.Variable(types.Name{Package: g.typesForGroupVersion[gv][0].Name.Package, Name: "SchemeGroupVersion"})
	...
	m := map[string]interface{}{
		"cacheGenericLister":         c.Institute.Type(cacheGenericLister),
		"cacheNewGenericLister":      c.Institute.Function(cacheNewGenericLister),
		"cacheSharedIndexInformer":   c.Institute.Type(cacheSharedIndexInformer),
		"groups":                     groups,
		"schemeGVs":                  schemeGVs,
		"schemaGroupResource":        c.Institute.Type(schemaGroupResource),
