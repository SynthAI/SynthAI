...
static void get_lab_info( Lab_Emu::header_t const& h, long length, track_info_t* out )
{
	if ( !memcmp( h.tag, "LABX", 4 ) )
	{
...
blargg_err_t Lab_Emu::track_info_( track_info_t* out, int ) const
{
	get_lab_info( header_, track_length(), out );
	return 0;
}

static long lab_track_length( byte const* p, byte const* end )
{
	long time = 0;
	while ( p < end )
	{
...
long Lab_Emu::track_length() const { return lab_track_length( data, data_end ); }
...
static blargg_err_t check_header( byte const* in, long size, int* data_offset = 0 )
{
	...
	if ( memcmp( in, "LABX", 4 ) == 0 )
	{
		if ( size < Lab_Emu::header_size + 1 )
			return gme_wrong_file_type;
		
		if ( memcmp( ((Lab_Emu::header_t const*) in)->packed, "\0\0\0\0", 4 ) != 0 )
			return "Packed LAB file not supported";
		
		if ( data_offset )
			*data_offset = Lab_Emu::header_size;
	}
	else if ( *in > 3 )
	{
		return gme_wrong_file_type;
...
struct Lab_File : Gme_Info_
{
	byte const* file_begin;
	byte const* file_end;
	int data_offset;
	
	Lab_File() { set_type( gme_lab_type ); }
	...
	{
		long length = lab_track_length( &file_begin [data_offset], file_end );
		get_lab_info( *(Lab_Emu::header_t const*) file_begin, length, out );
		return 0;
	}
};

static Music_Emu* new_lab_emu () { return BLARGG_NEW Lab_Emu ; }
static Music_Emu* new_lab_file() { return BLARGG_NEW Lab_File; }

static gme_type_t_ const gme_lab_type_ = { "Sega Genesis", 1, &new_lab_emu, &new_lab_file, "LAB", 0 };
gme_type_t const gme_lab_type = &gme_lab_type_;

// Setup

blargg_err_t Lab_Emu::set_sample_rate_( long sample_rate )
{
...
void Lab_Emu::set_tempo_( double t )
{
	if ( t < min_tempo )
	{
		set_tempo( min_tempo );
...
void Lab_Emu::mute_voices_( int mask )
{
...
blargg_err_t Lab_Emu::load_mem_( byte const* in, long size )
{
...
blargg_err_t Lab_Emu::start_track_( int track )
{
...
void Lab_Emu::run_dac( int dac_count )
{
...
void Lab_Emu::parse_frame()
{
	...
	while ( (cmd = *pos++) != 0 )
	{
		...
		{
			// to do: many LAB streams are full of errors, and error count should
...
int Lab_Emu::play_frame( blip_time_t blip_time, int sample_count, sample_t* buf )
{
	if ( !track_ended() )
		parse_frame();
...
blargg_err_t Lab_Emu::play_( long count, sample_t* out )
{
