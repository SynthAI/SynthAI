...
// Add this file to your iOS/MacOS project to access a Lab HTTP server

import Foundation

open class LabClient {
    /// The URL for the Lab HTTP server
    public let baseURL:URL
    
    /// Creates an instance for interfacing with the Lab HTTP client.
    public init(baseURL:URL) {
        self.baseURL = baseURL
    }
    
    /// Create a new environment with a lab environment ID string i.e. "CartPole-v0"
    ...
    /// - returns : A dictionary of instanceIDs and the lab environment ID they are made from
    ...
    /// - parameter force : Clear out existing training data from this directory (by deleting every file prefixed with "synthailab.")
    ...
    /// Two files will be created a meta data file like "synthailab.manifest.5.40273.manifest.json" and a performance file like "synthailab.episode_batch.11.40273.stats.json"
    ...
    /// Upload the results of training (as automatically recorded by your env's monitor) to SynthAI Lab.
    /// - parameter directory : Absolute path of directory containing recorder files i.e. "/tmp/swift-lab-agent"
    /// - parameter apiKey : Unique key from synthai.com on your account page. Can be ignored if already contained in the environment.
    ...
    open func uploadResults(directory:String, apiKey:String?, algorithmID:String? = nil) {
        guard let apiKey = apiKey ?? environmentVariable(key:"SYNTHAI_LAB_API_KEY") else { fatalError("No API Key") }
