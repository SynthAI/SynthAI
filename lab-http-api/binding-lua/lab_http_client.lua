...
function m.new(remote_base)
   ...
   setmetatable(self, {__index = LabClient})
   return self
end

function LabClient:parse_server_error_or_raise_for_status(resp)
...
function LabClient:get_request(route)
...
function LabClient:post_request(route, req_data)
   url = self.remote_base .. route
   options = {}
   options.content_type = 'application/json'
...
function LabClient:env_create(env_id)
   route = '/v1/envs/'
...
function LabClient:env_action_space_info(instance_id)
   route = '/v1/envs/'..instance_id..'/action_space/'
   resp_data = self:get_request(route)
   return resp_data['info']
end

function LabClient:env_action_space_sample(instance_id)
...
function LabClient:env_action_space_contains(instance_id)
...
function LabClient:env_observation_space_info(instance_id)
   route = '/v1/envs/'..instance_id..'/observation_space/'
   resp_data = self:get_request(route)
   return resp_data['info']
end

function LabClient:env_monitor_start(instance_id, directory, force, resume, video_callable)
...
function LabClient:env_monitor_close(instance_id)
   route = '/v1/envs/'..instance_id..'/monitor/close/'
   resp_data = self:post_request(route, '')
end

function LabClient:upload(training_dir, algorithm_id, api_key)
   if not api_key then
     api_key = os.getenv('SYNTHAI_LAB_API_KEY')
   end
...
function LabClient:shutdown_server()
   route = '/v1/shutdown/'
