...
pub type LabResult<T> = Result<T, hyper::Error>;
...
impl Environment {
	...
	pub fn reset(&mut self) -> LabResult<Vec<f64>> {
	...
	pub fn step(&mut self, action: Vec<f64>, render: bool) -> LabResult<State> {
	...
	pub fn monitor_start(&mut self, directory: String, force: bool, resume: bool) -> LabResult<()> {
		let mut req = BTreeMap::new();
	...
	pub fn monitor_stop(&mut self) -> LabResult<()> {
		let path = "/v1/envs/".to_string() + &self.instance_id + "/monitor/close/";
		try!(self.client.post(path, Value::Null));
		Ok(())
	}
	pub fn upload(&mut self, training_dir: String, api_key: String, algorithm_id: String) -> LabResult<()> {
		let mut req = BTreeMap::new();
...
impl LabClient {
    ...
    pub fn make(mut self, env_id: &str) -> LabResult<Environment> {
    ...
    pub fn get_envs(&mut self) -> LabResult<BTreeMap<String, String>> {
    	let json = try!(self.get("/v1/envs/".to_string()));
    	Ok(from_value(json.find("all_envs").unwrap().clone()).unwrap())
    }

    fn post(&mut self, route: String, request: Value) -> LabResult<Value> {
    ...
    fn get(&mut self, route: String) -> LabResult<Value> {
    	let url = self.address.clone() + &route;
