...
needs_api_key = pytest.mark.skipif(os.environ.get('SYNTHAI_LAB_API_KEY') is None, reason="needs SYNTHAI_LAB_API_KEY")
...
def test_create_destroy():
    client = lab_http_client.Client(get_remote_base())
...
def test_monitor_start_close_upload():
    assert os.environ.get('SYNTHAI_LAB_API_KEY')
...
def test_bad_instance_id():
    ...
    for call in try_these:
        try:
            call('bad_id')
        except lab_http_client.ServerError as e:
...
def test_missing_param_env_id():
    ...
    class BadClient(lab_http_client.Client):
...
#    client = lab_http_client.Client(get_remote_base())
#    cur_key = os.environ.get('SYNTHAI_LAB_API_KEY')
#    os.environ['SYNTHAI_LAB_API_KEY'] = ''
#    try:
#        print 'UPLOADING'
#        print cur_key
#        client.upload('tmp')
...
#    finally:
#        if cur_key:
#            os.environ['SYNTHAI_LAB_API_KEY'] = cur_key
