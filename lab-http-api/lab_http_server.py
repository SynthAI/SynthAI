...
class Envs(object):
    ...
    def monitor_start(self, instance_id, directory, force, resume, video_callable):
        ...
        self.envs[instance_id] = lab.wrappers.Monitor(env, directory, force=force, resume=resume, video_callable=v_c) 
...
def env_create():
    ...
    Parameters:
        - env_id: lab environment ID string, such as 'CartPole-v0'
...
def upload():
    ...
    your env's monitor) to SynthAI Lab.

    Parameters:
        - training_dir: A directory containing the results of a
        training run.
        - api_key: Your SynthAI API key
    ...
    try:
        lab.upload(training_dir, algorithm_id, writeup=None, api_key=api_key,
                   ignore_open_monitors=False)
        return ('', 204)
    except lab.error.AuthenticationError:
        raise InvalidUsage('You must provide an SynthAI Lab API key')
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Start a Lab HTTP API server')
