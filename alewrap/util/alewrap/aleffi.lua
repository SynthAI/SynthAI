...
ffi.cdef(readContent(paths.thisfile("alelua.inl")))
local lib = ffi.load(package.searchpath('libalelua',package.cpath))
...
function alelua.newAle(romPath)
...
function alelua.getRgbFromPalette(obs)
    obs = obs:contiguous()
