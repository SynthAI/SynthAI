...
function alelua.createEnv(romName, extraConfig)
    return alelua.AleEnv(romName, extraConfig)
end
...
local Env = torch.class('alelua.AleEnv')
function Env:__init(romPath, extraConfig)
    ...
    self.win = nil
    self.ale = alelua.newAle(romPath)
...
function Env:getRgbFromPalette(obs)
    return alelua.getRgbFromPalette(obs)
end
