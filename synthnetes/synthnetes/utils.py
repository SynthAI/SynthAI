...
import collections
import jinja2
import _jsunset as jsunset
import yaml
from distutils.version import StrictVersion

from synthnetes.version import VERSION
from synthnetes.errors import CompileError
...
def jsunset_file(file_path, **kwargs):
    """
    Evaluate file_path jsunset file.
    kwargs are documented in http://jsunset.org/implementation/bindings.html
    """
    try:
        return jsunset.evaluate_file(file_path, **kwargs)
    except Exception as e:
        logger.error("Jsunset error: failed to compile %s:\n %s", file_path, str(e))
        raise CompileError(e)


def prune_empty(d):
    '''Remove empty lists and empty dictionaries from x.
    (similar to jsunset std.prune but faster)
    '''
...
def check_version():
    '''
    Checks that the last version of synthnetes used is at least smaller or equal to current version.
    If the last version of synthnetes used is bigger, it will give instructions on how to upgrade and exit(1).
    '''
    if os.path.exists('.synthnetes'):
        with open('.synthnetes', 'r') as f:
            dot_synthnetes = yaml.safe_load(f)
            # If 'saved version is bigger than current version'
            if dot_synthnetes['version'] and StrictVersion(dot_synthnetes['version']) > StrictVersion(VERSION):
                print('{}Current version: {}'.format(termcolor.WARNING, VERSION))
                print('Last used version (in .synthnetes): {}{}\n'.format(dot_synthnetes["version"], termcolor.ENDC))
                print('Please upgrade synthnetes to at least "{}" in order to keep results consistent:\n'.format(dot_synthnetes["version"]))
                print('Docker: docker pull synthai/synthnetes')
                print('Pip (user): pip3 install --user --upgrade git+https://github.com/synthai/synthnetes.git --process-dependency-links\n')
                print('Check https://github.com/synthai/synthnetes#quickstart for more info.\n')
...
def save_version():
    '''
    Saves the current synthnetes version to a local .synthnetes file
    '''
    with open('.synthnetes', 'w') as f:
        dot_synthnetes = {'version': VERSION}
        yaml.safe_dump(dot_synthnetes, stream=f, default_flow_style=False)
