...
from synthnetes.resources import search_imports, resource_callbacks, inventory, inventory_reclass
from synthnetes.utils import jsunset_file, prune_empty, render_jinja2, PrettyDumper
from synthnetes.secrets import secret_gpg_raw_read, secret_token_from_tag, secret_token_attributes
from synthnetes.secrets import SECRET_TOKEN_TAG_PATTERN, secret_gpg_read, secret_gpg_write
from synthnetes.secrets import secret_gpg_exists, secret_gpg_write_function
from synthnetes.errors import SynthnetesError, CompileError, SecretError
from synthnetes import cached
...
def compile_targets(inventory_path, search_path, output_path, parallel, targets, **kwargs):
    ...
    temp_path = tempfile.mkdtemp(suffix='.synthnetes')
    ...
    try:
        if target_objs == []:
            ...
            raise SynthnetesError("Error: no targets found")
        ...
        if not isinstance(e, SynthnetesError):
            logger.error("\n~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~")
            traceback.print_exc()
        sys.exit(1)
    finally:
...
def load_target_inventory(inventory_path, targets):
    ...
    for target_name in targets_list:
        try:
            target_obj = inv['nodes'][target_name]['parameters']['synthnetes']
            valid_target_obj(target_obj)
            logger.debug("load_target_inventory: found valid synthnetes target %s", target_name)
            target_objs.append(target_obj)
        except KeyError:
            logger.debug("load_target_inventory: target %s has no synthnetes compile obj", target_name)
            pass
        ...
        if input_type == "jsunset":
            ...
            try:
                ...
                if ex.errno == errno.EEXIST:
                    pass
            output_type = obj["output_type"]  # output_type is mandatory in jsunset
            for input_path in input_paths:
                ...
                if os.path.exists(compile_file_sp):
                    ...
                    try:
                        compile_jsunset(compile_file_sp, _compile_path, search_path,
...
def compile_jsunset(file_path, compile_path, search_path, ext_vars, **kwargs):
    """
    Write file_path (jsunset evaluated) items as files to compile_path.
    Set output to write as json or yaml
    search_path and ext_vars will be passed as parameters to jsunset_file()
    kwargs:
    ...
    json_output = jsunset_file(file_path, import_callback=_search_imports,
...
class CompilingFile(object):
    ...
    def target_secret_func_write(self, token_path, func):
        ...
        if secrets_path is None:
            raise ValueError('secrets_path not set')
        recipients = target_inv['parameters']['synthnetes']['secrets']['recipients']
