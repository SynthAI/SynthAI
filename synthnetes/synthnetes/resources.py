...
from synthnetes.utils import render_jinja2_file, sha256_string
from synthnetes import __file__ as synthnetes_install_path
from synthnetes.errors import CompileError, InventoryError
import synthnetes.cached as cached
...
def resource_callbacks(search_path):
    ...
    on the native_callbacks keyword on jsunset
...
def jinja2_render_file(search_path, name, ctx):
    ...
    as there is a limitation with jsunset's native_callback approach:
    ...
    try:
        ...
        logger.error("Jsunset jinja2 failed to render %s: %s", _full_path, str(e))
...
def search_imports(cwd, import_str, search_path):
    """
    This is only to be used as a callback for the jsunset API!
    - cwd is the import context $CWD,
    - import_str is the "bla.jsunset" in 'import "bla.jsunset"'
    ...
    if not os.path.exists(full_import_path):
        install_path = os.path.dirname(synthnetes_install_path)
    ...
    # if the above search did not find anything, let jsunset error
