...
from synthnetes.utils import jsunset_file, PrettyDumper, flatten_dict, searchvar, deep_get, check_version, save_version
from synthnetes.targets import compile_targets
from synthnetes.resources import search_imports, resource_callbacks, inventory_reclass
from synthnetes.version import PROJECT_NAME, DESCRIPTION, VERSION
from synthnetes.secrets import secret_gpg_backend, secret_gpg_write, secret_gpg_reveal_file
from synthnetes.secrets import secret_gpg_reveal_dir, secret_gpg_reveal_raw, secret_gpg_update_recipients
from synthnetes.secrets import search_target_token_paths, secret_gpg_raw_read_fingerprints
from synthnetes.secrets import lookup_fingerprints
from synthnetes.errors import SynthnetesError
...
def main():
    ...
    eval_parser = subparser.add_parser('eval', help='evaluate jsunset file')
    eval_parser.add_argument('jsunset_file', type=str)
    ...
    compile_parser.add_argument('--no-prune', help='do not prune jsunset output',
                                action='store_true', default=False)
    ...
    compile_parser.add_argument('--ignore-version-check',
                                help='ignore the last synthnetes version used to compile (from .synthnetes)',
                                action='store_true', default=False)
    ...
    if cmd == 'eval':
        ...
        if args.write is not None:
            ...
            if args.target_name:
                inv = inventory_reclass(args.inventory_path)
                recipients = inv['nodes'][args.target_name]['parameters']['synthnetes']['secrets']['recipients']
            if args.file == '-':
                data = ''
            ...
            for target_name, token_paths in target_token_paths.items():
                try:
                    recipients = inv['nodes'][target_name]['parameters']['synthnetes']['secrets']['recipients']
