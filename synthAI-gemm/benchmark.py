...
from synthai_gemm import matmul
...
print "|     M|     N|     K| Op|SynthAI_32|cuBLAS_32|ratio_32|SynthAI_16|cuBLAS_16|ratio_16|"
print "|------|------|------|---|---------|---------|--------|---------|---------|--------|"
...
for m, n, k, at, bt in config:
    ...
    for dtype in ( np.float32, np.float16 ): #np.float32, np.float16,
        ...
        cublas = data.pop()
        synthai = sorted(data)[0]

        text = "%9.0f|%9.0f|%8.1f" % (synthai[1], cublas[1], synthai[1] / cublas[1])
