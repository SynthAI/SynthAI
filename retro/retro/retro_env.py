    ...
    def __init__(self, game, state=retro.STATE_DEFAULT, scenario=None, info=None, use_restricted_actions=retro.ACTIONS_FILTERED, record=False):
        ...
        if use_restricted_actions == retro.ACTIONS_DISCRETE:
            ...
            for combo in self.BUTTON_COMBOS:
                combos *= len(combo)
            self.action_space = lab.spaces.Discrete(combos)
        elif use_restricted_actions == retro.ACTIONS_MULTI_DISCRETE:
            self.action_space = lab.spaces.MultiDiscrete([len(combos) if lab_version >= (0, 9, 6) else (0, len(combos) - 1) for combos in self.BUTTON_COMBOS])
        else:
            self.action_space = lab.spaces.MultiBinary(self.NUM_BUTTONS)

        kwargs = {}
        if lab_version >= (0, 9, 6):
            kwargs['dtype'] = np.uint8
        self.observation_space = lab.spaces.Box(low=0, high=255, shape=img.shape, **kwargs)
    ...
    def render(self, mode='human', close=False):
        ...
        if mode == "rgb_array":
            ...
            if self.viewer is None:
                from lab.envs.classic_control.rendering import SimpleImageViewer
