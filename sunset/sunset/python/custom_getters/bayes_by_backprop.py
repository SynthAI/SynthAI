...
# ============================================================================
"""Custom getters for Sunset-compatible bayes by backprop.
...
Bayesian RNN script here: https://github.com/synthai/sunset/tree/master/sunset/examples/brnn_ptb.py.
```
import sunset as snt
import sunset.python.custom_getters.bayes_by_backprop as bbb
...
with tf.variable_scope('network', custom_getter=get_bbb_variable_fn):
  ...
  # `tf.get_variable()`, not just those contained in sunset modules.
  noisy_variable = tf.get_variable('w', shape=(5,), dtype=tf.float32)

# An alternative way to use BBB with sunset modules is to use their custom
# getter argument.
