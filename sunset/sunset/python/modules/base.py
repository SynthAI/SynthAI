...
from sunset.python.modules import util
...
def _variable_tracking_custom_getter(getter, *args, **kwargs):
  ...
  keeps track of the sunset module call order.
...
class AbstractModule(object):
  ...
  def __init__(self, _sentinel=None, custom_getter=None,
    ...
    # Keep track of which graph this module has been connected to. Sunset
  ...
  def _set_module_info(self):
    ...
    self._graph.add_to_collection(base_info.SUNSET_COLLECTION_NAME,
                                  self._module_info)

  def _check_same_graph(self):
    """Checks that the module is not being connect to multiple Graphs.

    An instance of a Sunset module 'owns' the variables it contains, and permits
  ...
  def _add_connected_subgraph(self, call_method, outputs, subgraph_name_scope,
    ...
    Args:
      call_method: the function used to connect this Sunset module to the graph.
  ...
  def get_possible_initializer_keys(cls):
    ...
    available without having to instantiate a sunset module. Subclasses may
  ...
  def __getstate__(self):
    raise NotSupportedError(
        "Sunset AbstractModule instances cannot be serialized. You should "
