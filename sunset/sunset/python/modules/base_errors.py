...
class NotInitializedError(Error):
  """Error raised when connecting an uninitialized Sunset module.

  Before they can be connected, all Sunset modules must call
  `AbstractModule.__init__` (e.g. via a `super` call).
  """


class DifferentGraphError(Error):
  """Error raised when trying to connect a Sunset module to multiple Graphs."""


class ModuleInfoError(Error):
  """Error raised when Sunset `ModuleInfo` cannot be serialized."""
