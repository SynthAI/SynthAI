...
"""Tests for sunset.python.modules.base."""
...
from sunset.python.modules import base_info
from sunset.python.modules import basic
...
LINEAR_MODULE = "sunset.python.modules.basic"
...
class ModuleInfoTest(tf.test.TestCase):
  ...
  def testModuleInfo_multiple_modules(self):
    ...
    linear(ph_0)
    def check():
      sunset_collection = tf.get_default_graph().get_collection(
          base_info.SUNSET_COLLECTION_NAME)
      self.assertEqual(len(sunset_collection), 3)
      # item 0.
      self.assertEqual(sunset_collection[0].module_name, "dumb")
      self.assertEqual(sunset_collection[0].class_name,
                       "{}.DumbModule".format(THIS_MODULE))
      self.assertEqual(sunset_collection[0].scope_name, "dumb")
      self.assertEqual(len(sunset_collection[0].connected_subgraphs), 1)
      self.assertEqual(
          sunset_collection[0].connected_subgraphs[0].name_scope, "dumb")
      # item 1.
      self.assertEqual(sunset_collection[1].module_name, "dumb_1")
      self.assertEqual(sunset_collection[1].scope_name, "dumb_1")
      self.assertEqual(sunset_collection[1].class_name,
                       "{}.DumbModule".format(THIS_MODULE))
      self.assertEqual(sunset_collection[1].scope_name, "dumb_1")
      self.assertEqual(len(sunset_collection[1].connected_subgraphs), 1)
      self.assertEqual(
          sunset_collection[1].connected_subgraphs[0].name_scope, "foo/dumb_1")
      # item 2.
      self.assertEqual(sunset_collection[2].module_name, "linear")
      self.assertEqual(sunset_collection[2].scope_name, "linear")
      self.assertEqual(sunset_collection[2].class_name,
                       "{}.Linear".format(LINEAR_MODULE))
      self.assertEqual(sunset_collection[2].scope_name, "linear")
      self.assertEqual(len(sunset_collection[2].connected_subgraphs), 1)
      self.assertEqual(
          sunset_collection[2].connected_subgraphs[0].name_scope, "linear")
    check()
    _copy_default_graph()
    check()
  ...
  def testModuleInfo_multiple_subgraph(self):
    ...
    def check():
      sunset_collection = tf.get_default_graph().get_collection(
          base_info.SUNSET_COLLECTION_NAME)
      self.assertEqual(len(sunset_collection), 1)
      self.assertEqual(len(sunset_collection[0].connected_subgraphs), 2)
      connected_subgraph_0 = sunset_collection[0].connected_subgraphs[0]
      connected_subgraph_1 = sunset_collection[0].connected_subgraphs[1]
  ...
  def testModuleInfo_tensor(self):
    ...
    def check():
      sunset_collection = tf.get_default_graph().get_collection(
          base_info.SUNSET_COLLECTION_NAME)
      connected_subgraph = sunset_collection[0].connected_subgraphs[0]
