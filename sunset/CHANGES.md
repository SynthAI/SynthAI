...
* `sunset.nest` `*iterable` functions now point to their equivalent from TF.
* Small documentation changes.
* Add `custom_getter` option to `sunset.Embed`.
...
* Added Sunset `ModuleInfo` to the "sunset" graph collection. This allows to
  ...
  can be used, for instance, to visualise the TensorFlow graph from a Sunset
  perspective.
...
* Removed Tensorflow dependencies in Bazel config files, which makes it
  unnecessary to have Tensorflow as a submodule of Sunset.
...
* Add IPython notebook that explains how Sunset's `BatchNorm` module can be
  configured.
...
Sunset op. This is in preparation for the C++ ops to be moved into tf/contrib.
...
* Sunset modules now throw an error if pickled. Instead of serializing module
...
the program.
* Sunset no longer allows the possibility that `self._graph` does not exist.
