...
def atari_vnc_wrapper(env):
    ...
    env = wrappers.LabCoreAction(env)
    return env
...
specs = [
    (lab.spec('lab-core.PongDeterministicSync-v3'), AtariMatcher(), atari_vnc_wrapper),
    (lab.spec('lab-core.PitfallDeterministicSync-v3'), AtariMatcher(), atari_vnc_wrapper),

    # This test is still broken. Looks like we're not piping the seed
    # to the CartPole env behind VNC
#    (lab.spec('lab-core.CartPoleLowDSync-v0'), CartPoleLowDMatcher())
]
