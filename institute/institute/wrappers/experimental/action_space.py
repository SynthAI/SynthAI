...
from institute.wrappers.lab_core import lab_core_action_space
...
class SafeActionSpace(vectorized.Wrapper):
    """
    Recall that every institute environment receives a list of VNC events as action.
    ...
    def __init__(self, env):
        ...
        if self.spec.tags.get('runtime') == 'lab-core':
            self.action_space = lab_core_action_space(self.spec._kwargs['lab_core_id'])
...
class SoftmaxClickMouse(vectorized.ActionWrapper):
    ...
    def __init__(self, env, active_region=(10, 75 + 50, 10 + 160, 75 + 210), discrete_mouse_step=10, noclick_regions=[]):
        ...
        self.action_space = lab.spaces.Discrete(len(self._points))
