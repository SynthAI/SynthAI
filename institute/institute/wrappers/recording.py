...
from institute import rewarder, spaces, vectorized, pyprofile
from institute.utils import random_alphanumeric
...
The institute-viewer project (http://github.com/synthai/institute-viewer) provides a browser-based UI
for examining logs.
...
class RecordingWriter(object):
    ...
    def __init__(self, recording_dir, instance_id, channel_id, async_write=True):
        self.log_fn = 'institute.recording.{}.{}.{}.jsonl'.format(os.getpid(), instance_id, channel_id)
        log_path = os.path.join(recording_dir, self.log_fn)
        self.bin_fn = 'institute.recording.{}.{}.{}.bin'.format(os.getpid(), instance_id, channel_id)
