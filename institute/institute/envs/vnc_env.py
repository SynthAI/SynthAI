...
from institute import error, pyprofile, rewarder, spaces, twisty, vectorized, vncdriver
from institute import remotes as remotes_module
from institute.envs import diagnostics
from institute.runtimes import registration
from institute.vncdriver import libvnc_session
...
extra_logger = logging.getLogger('institute.extra.'+__name__)

if os.environ.get('INSTITUTE_VNCDRIVER', None) == 'go':
...
def vnc_session(which=None):
    ...
    if which is None:
        which = os.environ.get('INSTITUTE_VNCDRIVER')
    ...
    if which == 'go':
        ...
        try:
            ...
            logger.info("Using pure Python vncdriver implementation. Run 'pip install go-vncdriver' to install the more performant Go implementation. Optionally set the environment variable INSTITUTE_VNCDRIVER='go' to force its use.")
            return py_vncdriver()
    else:
...
class VNCEnv(vectorized.Env):
    ...
    def configure(self, remotes=None,
        ...
        """Institute method to configure the environment.
        ...
        if remotes is None:
            remotes = os.environ.get('LAB_VNC_REMOTES', '1')
