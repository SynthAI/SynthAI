...
import re
from institute import error, pyprofile, spaces
...
class DiagnosticsInstance(object):
    ...
    def add_metadata(self, observation, info, available_at=None):
        ...
        if self.network is None:
            ...
            # same machine as the VNC server, such as the institute
        ...
        if metadata is not None:
            ...
            if probe_received_at == 0 or self.disable_action_probes:
                ...
                logger.info('[%s] Probe is marked as received at %s, but probe_sent_at is None. This is surprising. (HINT: do you have multiple institute instances talking to the same environment?)', self.label, probe_received_at)
...
#         """TODO: This needs be moved to institute-envs and run there. Otherwise it only works if the env and agent
