...
class LabCoreEnv(vnc_env.VNCEnv):
    def __init__(self, lab_core_id, fps=60):
        super(LabCoreEnv, self).__init__()

        self.metadata = dict(self.metadata)
        self.metadata['video.frames_per_second'] = fps

        self.lab_core_id = lab_core_id
        self._seed_value = None

        self.vnc_pixels = True

class LabCoreSyncEnv(LabCoreEnv):
    ...
    the VNC session, but it also supports receiving the normal Lab
    observations over the rewarder socket.)

    Provided primarily for testing and debugging.
    """

    def __init__(self, lab_core_id, fps=60, vnc_pixels=True):
        super(LabCoreSyncEnv, self).__init__(lab_core_id, fps=fps)
        ...
        if not vnc_pixels:
            self._core_env = lab.spec(lab_core_id).make()
        else:
            self._core_env = None
