...
from institute.remotes.allocator_remote import AllocatorManager
from institute.remotes.docker_remote import DockerManager
from institute.remotes.hardcoded_addresses import HardcodedAddresses
...
def build(client_id, remotes, runtime=None, start_timeout=None, **kwargs):
    ...
    if re.search('^\d+$', remotes): # an integer, like -r 20
        ...
        if runtime is None:
            raise error.Error('Must provide a runtime. HINT: try creating your env instance via lab.make("flashgames.DuskDrive-v0")')
