...
from lab.utils import closer
from institute import error
from institute.remotes import healthcheck, remote
from institute import error, utils
from institute.remotes.compose import container, log_printer, progress_stream
...
class DockerManager(object):
    ...
    def _start(self):
        ...
        if int(os.environ.get('SYNTHAI_REMOTE_VERBOSE', '1')) > 0:
...
class PortAssigner(object):
    def __init__(self, reuse=False):
        self.reuse = reuse
        self.instance_id = 'institute-' + random_alphanumeric(length=6)
...
class DockerInstance(object):
    ...
    def _spawn(self):
        ...
        try:
            ...
            self._pull_image()
            # If we called pull_image from multiple processes (as we do with institute-starter-agent A3C)
    ...
    def _remove(self):
        ...
        try:
            ...
            if 'device or resource busy' in str(e.explanation):
                logger.info("[%s] Could not remove container: %s. You can always kill all automanaged environments on this Docker daemon via: docker rm -f $(docker ps -q -a -f 'label=com.synthai.automanaged=true')", self.label, e)
                self._container_id = None
                return e
            else:
                raise
...
if __name__ == '__main__':
    ...
    from institute.runtimes import registration

    # docker run --name test --rm -ti -p 5900:5900 -p 15900:15900 quay.io/synthai/institute.lab-core
    instance = DockerManager(
        runtime=registration.runtime_spec('lab-core'),
        n=2,
    )
    instance.start()
