...
import logging
import os
from institute import pyprofile
import sys
...
from institute.twisty import reactor
...
class RewarderProtocol(websocket.WebSocketServerProtocol):
    ...
    def onConnect(self, request):
        if not os.path.exists('/usr/local/synthai/privileged_state/password'):
            raise error.Error('No such file: /usr/local/synthai/privileged_state/password. (HINT: did the init script run /app/institute-envs/base/synthai-setpassword?)')
        with open('/usr/local/synthai/privileged_state/password') as f:
            password = f.read().strip()

        self._message_id = 0
        self._request = request
        self._observer = request.headers.get('synthai-observer') == 'true'
    ...
    def authenticate(self, request):
        ...
        if username != self.password:
            ...
            self.reject('Invalid password: {!r}. If you are using the allocator, you should see your password in the logs; if spinning up an environment by hand, it defaults to "synthai". Connect as vnc://<ip>:<port>?password=<password>.'.format(username))
            return
