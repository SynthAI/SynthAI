...
The majority of the remotes for Institute environments run inside
...
- Create an **automatic local remote** using ``env.configure(remotes=1)``.
  In this case, ``institute`` automatically creates a remote locally by spinning
...
This will download the ``quay.io/synthai/institute.flashgames`` docker
container and start 1 copy of it locally.

.. code:: python

    import lab
    import institute # register the institute environments
...
The Institute remote will use ``$DOCKER_NET_HOST`` when connecting to the VNC and rewarder ports.
...
<https://github.com/synthai/institute/blob/master/institute/runtimes/__init__.py>`__
...
.. code:: shell
	...
	image=quay.io/synthai/institute.flashgames:0.19.19. Run the same thing by hand as:
	docker run -p 10000:5900 -p 10001:15900 --cap-add NET_ADMIN --cap-add SYS_ADMIN
	--ipc host quay.io/synthai/institute.flashgames:0.19.19
...
If you have an AWS account, you can spin up a **starter Docker cluster** to host your own remotes. First click the "Launch Stack" button and follow the steps on the AWS console to deploy your cluster.
  ...
  .. image:: https://s3.amazonaws.com/cloudformation-examples/cloudformation-launch-stack.png
     :target: https://console.aws.amazon.com/cloudformation/home#/stacks/new?stackName=SynthAI-Institute&templateURL=thttps://s3-us-west-2.amazonaws.com/synthai-public/institute/starter-cluster-cf-0.1.0.json
...
Once your stack on AWS is ready, run `starter-cluster` to start your environments
  ...
  .. code:: shell
    ...
    $ example/starter-cluster/starter-cluster start -s [stack-name] -i [path-to-ssh-key] \
        --runtime [institute-runtime] -n [number-of-envs]
...
or example, the follow will start two flashgames remotes:
  ...
  .. code:: shell
    ...
    $ bin/starter-cluster -v start -s SynthAI-Institute -i my-ec2-key.pem -r flashgames -n 2
    Creating network "flashgames_default" with the default driver
    Pulling flashgames-0 (quay.io/synthai/institute.flashgames:0.19.36)...
    ip-172-33-1-4: Pulling quay.io/synthai/institute.flashgames:0.19.36... : downloaded
    ip-172-33-28-242: Pulling quay.io/synthai/institute.flashgames:0.19.36... : downloaded
...
Running ``bin/starter-cluster start`` again will restart your remotes. To stop them, run:
  ...
  .. code:: shell
    ...
    $ bin/starter-cluster stop -s SynthAI-Institute -i my-ec2-key.pem -r flashgames
...
.. code:: shell
  ...
  $ bin/starter-cluster -v start -s SynthAI-Institute -i my-ec2-key.pem -r flashgames   -n 2
    ...
    ip-172-33-1-4: Pulling quay.io/synthai/institute.flashgames:0.19.36... : downloaded
    ip-172-33-28-242: Pulling quay.io/synthai/institute.flashgames:0.19.36... :   downloaded
    ip-172-33-9-51: Pulling quay.io/synthai/institute.flashgames:0.19.36... :   downloaded
    ip-172-33-27-141: Pulling quay.io/synthai/institute.flashgames:0.19.36... :   downloaded
...
`institute/runtimes/__init__.py <https://github.com/synthai/institute/blob/master/institute/runtimes/__init__.py>`__,
and the specific version number tags for the corresponding Docker
images are specified in
`runtimes.yml <https://github.com/synthai/institute/blob/master/institute/runtimes.yml>`__.
