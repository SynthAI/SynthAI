...
try:
    ...
    def data_path():
        raise RuntimeError('Could not find Lab Retro data directory')
...
def run(game, state=None, entry=None, **kwargs):
    ...
    remote_name = kwargs.get('remote_env', 'synthai/retro-env')
    agent_command = []
...
def build(path, tag, install=None, pass_env=False):
    ...
    docker_file = ['FROM synthai/retro-agent',
...
def init_parser(subparsers):
    ...
    parser_run.add_argument('--use-host-data', '-d', action='store_true', help='Use the host Lab Retro data directory')
...
def main(argv=sys.argv[1:]):
    parser = argparse.ArgumentParser(description='Run SynthAI Retro Contest support code')
