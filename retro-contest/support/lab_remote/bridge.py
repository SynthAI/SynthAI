...
lab_version = tuple(int(x) for x in lab.__version__.split('.'))
...
class Bridge:
    ...
    def __init__(self, base):
        ...
        def close(message):
            ...
            if 'exception' in message:
                import lab_remote.exceptions as gre
    ...
    def wrap(self, name, space):
        channel = None
        if isinstance(space, lab.spaces.MultiBinary):
            if space.n < 64:
        ...
        elif isinstance(space, lab.spaces.Discrete):
            channel = IntChannel()
            channel.annotate('n', space.n)
            channel.annotate('type', 'Discrete')
        elif isinstance(space, lab.spaces.MultiDiscrete):
            if lab_version >= (0, 9, 6):
        ...
        elif isinstance(space, lab.spaces.Box):
    ...
    def unwrap(space):
        if space.annotations['type'] == 'MultiBinary':
            return lab.spaces.MultiBinary(int(space.annotations['n']))
        if space.annotations['type'] == 'Discrete':
            return lab.spaces.Discrete(int(space.annotations['n']))
        if space.annotations['type'] == 'MultiDiscrete':
            if lab_version >= (0, 9, 6):
                return lab.spaces.MultiDiscrete(space.shape[0])
            else:
                return lab.spaces.MultiDiscrete(space.shape)
        if space.annotations['type'] == 'Box':
            kwargs = {}
            if lab_version >= (0, 9, 6):
                kwargs['dtype'] = space.dtype
            return lab.spaces.Box(low=0, high=255, shape=space.shape, **kwargs)
